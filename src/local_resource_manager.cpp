...
namespace p2p {
    bool LocalResourceManager::addResource(const std::string& new_resource_name, const std::string& new_resource_path) {
        ...
        ResourceInfo resource_info {
            ...
            .size = std::filesystem::file_size(new_resource_path),
    ...
    std::optional<std::string> LocalResourceManager::getResourcePath(const std::string& name) const {
    ...
    std::map<std::string, ResourceInfo> LocalResourceManager::getAllResources() const {
        std::shared_lock lock(mutex_);
    ...
    std::optional<ResourceInfo> LocalResourceManager::getResourceInfo(const std::string& name) const {
        std::shared_lock lock(mutex_);
