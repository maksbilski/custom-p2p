...
TEST_F(LocalResourceManagerThreadTest, ConcurrentReads) {
    ...
    for (int i = 0; i < NUM_THREADS; ++i) {
        ASSERT_TRUE(manager.addResource("resource_" + std::to_string(i), temp_paths_[i]));
    }
    ...
    for (int i = 0; i < NUM_THREADS; ++i) {
        threads.emplace_back([&manager, &successful_reads, i]() {
            for (int j = 0; j < OPERATIONS_PER_THREAD; ++j) {
                ...
                auto resource_path = manager.getResourcePath("resource_" + std::to_string(i));
...
TEST_F(LocalResourceManagerThreadTest, ConcurrentWrites) {
    ...
    for (int i = 0; i < NUM_THREADS; ++i) {
        threads.emplace_back([&manager, &successful_adds, &successful_removes, i, this]() { for (int j = 0; j < OPERATIONS_PER_THREAD; ++j) {
                std::string resource_name = "resource_" + std::to_string(i) + "_" + std::to_string(j);
...
TEST_F(LocalResourceManagerThreadTest, ConcurrentAccessToSameResource) {
    ...
    for (int i = 0; i < NUM_THREADS; ++i) {
        threads.emplace_back([&manager, &successful_operations, &shared_resource_name]() {
            for (int j = 0; j < OPERATIONS_PER_THREAD; ++j) {
                switch (j % 3) {
                    case 0:
