...
protected:
  ...
  bool compareFiles(const std::string &file1, const std::string &file2) {
    ...
    while (f1 && f2) {
      ...
      if (count1 != count2) {
        std::cout << "Files have different sizes at position "
                  << total_bytes_compared << std::endl;
        return false;
      }
      if (memcmp(buf1.data(), buf2.data(), count1) != 0) {
        std::cout << "Content mismatch at position " << total_bytes_compared
                  << std::endl;
        return false;
      }
    }
    ...
    std::cout << "File comparison complete. Total bytes compared: "
...
TEST_F(ResourceDownloaderTest, CanDownloadExistingResource) {
  ...
  std::cout << "Download result: " << (result ? "success" : "failure")
            << std::endl;
  EXPECT_TRUE(result);
  ...
  std::cout << "Checking if downloaded file exists: " << downloaded_file
            << std::endl;
...
TEST_F(ResourceDownloaderTest, ConcurrentDownloadsStressTest) {
  ...
  std::cout << "Starting " << NUM_CLIENTS << " concurrent downloads..."
            << std::endl;
  ...
  std::cout << "All downloads completed in " << duration.count() << "ms"
            << std::endl;
  ...
  std::cout << "Concurrent download test completed. " << successful_comparisons
            << "/" << NUM_CLIENTS << " files verified successfully."
            << std::endl;
}
