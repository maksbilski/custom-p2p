...
namespace p2p {
    ...
    int ResourceDownloader::initialize_socket_(const std::string& host, int port) const {
    ...
    void ResourceDownloader::send_resource_request_(
        ...
        while (total_sent < request->messageLength) {
            ssize_t bytes_sent = send(sock, 
                                    buffer + total_sent, 
                                    request->messageLength - total_sent, 
                                    0);
    ...
    std::pair<bool, uint64_t> ResourceDownloader::receive_initial_response_(int sock) const {
        uint8_t status;
    ...
    void ResourceDownloader::receive_file_(
        int sock, 
          const std::string& resource_name,
          uint64_t file_size
      ) const {
          std::filesystem::path file_path = std::filesystem::path(download_dir_) / resource_name;
          ...
          while (total_received < file_size) {
              size_t to_receive = std::min(sizeof(buffer), 
